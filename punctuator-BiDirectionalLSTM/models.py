...
class LSTM(tf.keras.Model):
    ...
    def set_weights(self, weights):
        assert len(weights) == len(self.params), "Number of provided weights must match the number of model parameters"
        for param, weight in zip(self.params, weights):
            param.assign(weight)

    def get_weights(self):
        return [param.numpy() for param in self.params]
...
def saveOLD(model, file_path, learning_rate=None, validation_ppl_history=None, best_validation_ppl=None, epoch=None, random_state=None):
    import pickle
    state = {
        "type":                     model.__class__.__name__,
        "n_hidden":                 model.n_hidden,
